...
class GraphqlController < ApplicationController
  ...
  def execute(user_login = params[:user_login].to_s.strip)
    if user_login.match?(/\A[A-Za-z0-9_-]+\z/)
      ...
      context = {}
      result = Cyber2Schema.execute(query, variables:, context:, operation_name:)
      result = result.to_h

      @name = if !execute_name['errors']
                eval(execute_name['data']['name'])
              else
                ''
              end


      @result = if !result['errors']
                  eval(result['data']['repos'])
                else
                  ''
                end
